...
import type { CatchKind } from "@/types/database";

/**
 * 캐치 / 유저클립 공용 아카이브 화면.
 *
 * 두 페이지는 보여주는 종류만 다르고 필터·정렬·묶어보기는 완전히 같아서
 * 화면을 복사하지 않고 kind 와 문구만 주입해서 쓴다.
 */
...
export interface CatchArchiveSearchParams {
  member?: string;
  sort?: string;
  period?: string;
  page?: string;
...
export async function CatchArchive({
  kind,
  basePath,
  title,
  emptyLabel,
  ...
  basePath: string;
  title: string;
  /** "캐치" / "유저클립" 처럼 빈 상태 문구에 넣을 명칭 */
  emptyLabel: string;
  searchParams: CatchArchiveSearchParams;
}) {
    ...
    return (
      <>
        ...
        {grouped.data.length === 0 ? (
          <EmptyState
            title={`${emptyLabel}가 없습니다`}
            description="기간을 넓히거나 수집기가 적재했는지 확인해 주세요."
          />
        ) : (
  ...
  return (
    <>
      <PageHeader
        title={title}
        description="원본 콘텐츠는 복제하지 않습니다. 누르면 원본으로 이동합니다."
      />
      {filters}
      ...
      {result.data.rows.length === 0 ? (
        <EmptyState title={`${emptyLabel}가 없습니다`} />
      ) : (
